 ...
 * record with no tail padding: three of the five on LP64, and all five on i386,
...
int main(void)
{
    ...
    for (const ASS_LayoutEvent *e = result->layout->events; e; e = e->next) {
        ...
        for (const ASS_LayoutUnit *u = e->units; u; u = u->next) {
            ...
            for (const ASS_LayoutOutline *o = u->fill; o; o = o->next)
                ASSERT_SIZE_TAG(ASS_LayoutOutline, next, o->struct_size);
        }
    }
