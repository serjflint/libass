...
static ASS_LayoutRect nonzero_image_bounds(ASS_Image *image)
{
    int left = INT_MAX, top = INT_MAX, right = INT_MIN, bottom = INT_MIN;
    for (; image; image = image->next) {
        if ((image->color & 0xff) == 0xff)
            continue;
        for (int y = 0; y < image->h; y++) {
            const uint8_t *row = image->bitmap + y * image->stride;
            for (int x = 0; x < image->w; x++) {
                if (!row[x])
                    continue;
                left = left < image->dst_x + x ? left : image->dst_x + x;
                top = top < image->dst_y + y ? top : image->dst_y + y;
                right = right > image->dst_x + x + 1 ? right : image->dst_x + x + 1;
                bottom = bottom > image->dst_y + y + 1 ? bottom : image->dst_y + y + 1;
            }
        }
    }
    if (left == INT_MAX)
...
static void validate_outlines(ASS_LayoutOutline *outline)
{
    for (; outline; outline = outline->next) {
        assert(outline->struct_size == sizeof(*outline));
        for (size_t i = 0; i < outline->point_count; i++) {
            assert(isfinite(outline->points[i].x));
            assert(isfinite(outline->points[i].y));
            assert(fabs(outline->points[i].x) < 1000);
            assert(fabs(outline->points[i].y) < 1000);
        }
    }
}

static double rightmost_cluster_ink(const ASS_LayoutEvent *metrics)
{
    double right = -HUGE_VAL;
    for (ASS_LayoutUnit *cluster = metrics->units;
         cluster; cluster = cluster->next) {
        for (ASS_LayoutOutline *outline = cluster->fill;
             outline; outline = outline->next) {
            for (size_t i = 0; i < outline->point_count; i++)
                right = fmax(right, cluster->pos.x + outline->points[i].x);
        }
    }
    return right;
}
...
static ASS_LayoutRect cluster_ink_bounds(const ASS_LayoutUnit *cluster)
{
    double left = HUGE_VAL, top = HUGE_VAL;
    double right = -HUGE_VAL, bottom = -HUGE_VAL;
    for (ASS_LayoutOutline *outline = cluster->fill;
         outline; outline = outline->next) {
        for (size_t i = 0; i < outline->point_count; i++) {
            double x = cluster->pos.x + outline->points[i].x;
            double y = cluster->pos.y + outline->points[i].y;
            left = fmin(left, x);
            top = fmin(top, y);
            right = fmax(right, x);
    ...
    assert(isfinite(left) && isfinite(top) && isfinite(right) && isfinite(bottom));
    return (ASS_LayoutRect) {
        floor(left), floor(top), ceil(right) - floor(left),
        ceil(bottom) - floor(top),
    };
}
...
static size_t validate_metrics(ASS_LayoutEvent *metrics)
{
    ...
    for (; metrics; metrics = metrics->next) {
        ...
        for (ASS_LayoutUnit *cluster = metrics->units;
            ...
            validate_outlines(cluster->fill);
...
int main(void)
{
    ...
    ASS_LayoutRequest logical_only = valid_limits();
    logical_only.flags = 0;
    logical_only.max_outlines = 0;
    logical_only.max_outline_points = 0;
    ASS_LayoutStatus status = ASS_LAYOUT_OK;
    with_metrics = render_bounded(renderer, track, 1000, &logical_only,
                                  &metrics, &status);
    assert(with_metrics);
    assert(status == ASS_LAYOUT_OK);
    for (ASS_LayoutUnit *unit = metrics->units; unit; unit = unit->next)
        assert(!unit->fill);
    ...
    const size_t limit_offsets[] = {
        ...
        offsetof(ASS_LayoutRequest, max_outlines),
        offsetof(ASS_LayoutRequest, max_outline_points),
    ...
    double early_transform_right = rightmost_cluster_ink(metrics);
    ...
    assert(rightmost_cluster_ink(metrics) != early_transform_right);
    ...
    assert(rightmost_cluster_ink(metrics) >
           metrics->bitmap_bounds.x + metrics->bitmap_bounds.w);
    ...
    ASS_LayoutRect ink = cluster_ink_bounds(metrics->units);
    pixels = nonzero_image_bounds(with_metrics);
    assert(fabs(ink.x - pixels.x) <= 1);
    assert(fabs(ink.y - pixels.y) <= 1);
    assert(fabs(ink.x + ink.w - pixels.x - pixels.w) <= 1);
    assert(fabs(ink.y + ink.h - pixels.y - pixels.h) <= 1);
